...
export function CheckInModal({ item, onClose, onSuccess }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="bg-[#151515] border border-[#333] rounded-lg w-full max-w-lg flex flex-col">
        ...
        <form onSubmit={handleCheckIn} className="p-6 space-y-8">
          ...
          <div>
            ...
            <div className="relative">
              <div className="w-full bg-[#111] border border-[#333] p-3 text-sm text-white mb-2 rounded font-bold truncate">
                {selectedLocLabel}
              </div>
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full bg-[#111] border border-[#333] pl-10 pr-4 py-2 text-sm text-brand-text focus:border-brand-accent focus:outline-none transition-colors"
                />
              </div>

              {locSearch && (
                <div className="absolute z-10 w-full mt-1 bg-[#1a1a1a] border border-[#333] max-h-48 overflow-y-auto rounded shadow-xl">
                  {locResults.map(loc => (
                    <button 
                      key={loc.id} 
                      type="button"
                      onClick={() => handleSelectLoc(loc.id, loc.pathLabel)}
                      className="w-full text-left p-3 text-sm text-brand-text hover:bg-[#222] hover:text-white border-b border-[#333] last:border-0 truncate"
                    >
                      {loc.pathLabel}
                    </button>
                  ))}
